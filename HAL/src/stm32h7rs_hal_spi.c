...
void HAL_SPI_IRQHandler(SPI_Handle *handle)
{
    ...
    if (HAL_IS_BIT_CLR(trigger, SPI_SR_OVR) && HAL_IS_BIT_CLR(trigger, SPI_SR_UDR) && HAL_IS_BIT_SET(trigger, SPI_SR_DXP)) {
        handle->txISR(handle);
        handle->rxISR(handle);
        handled = 1UL;
    }
    ...
    if (HAL_IS_BIT_CLR(trigger, SPI_SR_OVR) && HAL_IS_BIT_SET(trigger, SPI_SR_RXP) && HAL_IS_BIT_CLR(trigger, SPI_SR_DXP)) {
        handle->rxISR(handle);
        handled = 1UL;
    }

    /* SPI in mode Transmitter -------------------------------------------------*/
    if (HAL_IS_BIT_CLR(trigger, SPI_SR_UDR) && HAL_IS_BIT_SET(trigger, SPI_SR_TXP) && HAL_IS_BIT_CLR(trigger, SPI_SR_DXP)) {
        handle->txISR(handle);
        handled = 1UL;
    }
    ...
    if ((trigger & (SPI_SR_MODF | SPI_SR_OVR | SPI_SR_TIFRE | SPI_SR_UDR)) != 0UL) {
        ...
        if (handle->ErrorCode != HAL_SPI_ERROR_NONE) {
            ...
            handle->Instance->IER &=
                ~(SPI_IER_EOTIE | SPI_IER_RXPIE | SPI_IER_TXPIE | SPI_IER_MODFIE | SPI_IER_OVRIE | SPI_IER_TIFREIE | SPI_IER_UDRIE);
...
static void SPI_CloseTransfer(SPI_Handle *handle)
{
    ...
    /* Disable ITs */
    handle->Instance->IER &=
        ~(SPI_IER_EOTIE | SPI_IER_TXPIE | SPI_IER_RXPIE | SPI_IER_DXPIE | SPI_IER_UDRIE | SPI_IER_OVRIE | SPI_IER_TIFREIE | SPI_IER_MODFIE);
