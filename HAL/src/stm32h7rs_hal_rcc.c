...
HAL_Status HAL_RCC_OscConfig(RCC_OscInit *RCC_OscInitStruct)
{
    ...
    if (RCC_OscInitStruct->HSIState != RCC_HSI_OFF) {
        ...
        if (RCC_OscInitStruct->HSICalibration != 0) {
            MODIFY_REG(RCC->HSICFGR, RCC_HSICFGR_HSITRIM, RCC_OscInitStruct->HSICalibration << RCC_HSICFGR_HSITRIM_Pos);
        }
    } else {
...
HAL_Status HAL_RCC_PLLConfig(RCC_PLLInit *RCC_PLLInitStruct)
{
    ...
    if (RCC_PLLInitStruct->PLLState == RCC_PLL_ON) {
        ...
        MODIFY_REG(RCC->PLLCKSELR, RCC_PLLCKSELR_DIVM1, RCC_PLLInitStruct->PLLM << RCC_PLLCKSELR_DIVM1_Pos);

        // Configure PLLN (multiplier)
        MODIFY_REG(RCC->PLL1DIVR1, RCC_PLL1DIVR1_DIVN, (RCC_PLLInitStruct->PLLN - 1U) << RCC_PLL1DIVR1_DIVN_Pos);

        // Configure PLLP (divider for system clock output)
        MODIFY_REG(RCC->PLL1DIVR1, RCC_PLL1DIVR1_DIVP, (RCC_PLLInitStruct->PLLP - 1U) << RCC_PLL1DIVR1_DIVP_Pos);

        // Configure PLLQ (divider for peripherals)
        MODIFY_REG(RCC->PLL1DIVR1, RCC_PLL1DIVR1_DIVQ, (RCC_PLLInitStruct->PLLQ - 1U) << RCC_PLL1DIVR1_DIVQ_Pos);

        // Configure PLLR (divider)
        MODIFY_REG(RCC->PLL1DIVR1, RCC_PLL1DIVR1_DIVR, (RCC_PLLInitStruct->PLLR - 1U) << RCC_PLL1DIVR1_DIVR_Pos);
